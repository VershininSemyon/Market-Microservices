...
.PHONY: help up down restart build rebuild ps \
	logs logs-auth logs-products logs-reviews logs-consumer logs-notification \
	logs-gateway logs-auth-postgres logs-products-postgres logs-reviews-postgres \
	...
	logs-elasticvue \
	sh-auth sh-products sh-reviews sh-consumer sh-notification \
	psql-auth psql-products psql-reviews \
	redis-auth redis-products \
	migrate-auth migrate-products migrate-reviews \
	migrate-create-auth migrate-create-products migrate-create-reviews \
	ruff-auth ruff-products ruff-reviews ruff-all \
	mypy-auth mypy-products mypy-reviews mypy-all \
	lint-auth lint-products lint-reviews lint-all \
	up-auth up-products up-reviews up-notification \
	down-auth down-products down-reviews down-notification \
	restart-auth restart-products restart-reviews restart-notification \
	restart-infra
